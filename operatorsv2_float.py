...
from nqs_vmcore_complex_float import SampleBuffer
...
class cal_op():
    def __init__(self, **kwargs):
        ...
        self.theta_model = kwargs.get('theta_model')
    ...
    def get_sample(self, op):
        self._sampler = MCsampler(state_size=self._state_size, model=self.logphi_model, get_init_state=self._get_init_state, 
    ...
    def _ops(self, sample_division):
        ...
        with torch.no_grad():
            ...
            logphi_ops = self.logphi_model(uss.float()).reshape(n_sample, n_updates)
            theta_ops = self.theta_model(uss.float()).reshape(n_sample, n_updates)

            logphi = self.logphi_model(states.float()).reshape(len(states),-1)
            theta = self.theta_model(states.float()).reshape(len(states),-1)
