...
class MCsampler(): 
    ...
    def _mh_sampler(self, n_sample_per_thread: int, state0, seed_number):
        ...
        with torch.no_grad():
            ...
            while i < n_sample_per_thread:
                state, logphi, theta, update_states, update_coeffs = self.get_single_sample(state, logphi, theta, masks[i], rands[i], update_states, update_coeffs)
    ...
    def parallel_mh_sampler(self,threads=4):
        ...
        self._state0 = self._get_init_state(self._state_size, kind=self._init_type, n_size=threads)
        ...
        theta_list = np.zeros([threads, n_sample_per_thread])
