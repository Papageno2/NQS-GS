...
def train(epochs=100, Ops_args=dict(), Ham_args=dict(), n_sample=100, init_type='rand', n_optimize=10,
    ...
    def update(IntCount, learning_rate, epsilon, n_step):
        ...
        for i in range(n_step):
            psi_model.zero_grad()
            wn, dt = update_one_step(data, learning_rate, epsilon)
            t += dt
    ...
    for epoch in range(epochs):
        ...
        WsN = update(IntCount, learning_rate, epsilon, n_optimize)
        ...
        logger.info('Epoch: {}, AvgE: {:.5f}, StdE: {:.5f}, Lr: {:.5f}, Ep: {:.5f}, WsN: {:.5f}, IntCount: {}, SampleTime: {:.3f}, OptimTime: {:.3f}, TolTime: {:.3f}'.
